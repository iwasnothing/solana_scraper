...
while true ; do
   ...
   dt=$(docker exec $(docker ps|grep rust_client_neo4jdb_1|awk '{print $1}') bin/cypher-shell -u neo4j -p 94077079 "MATCH (a:Account)-[r:TRANSFER_TO]->(b:Account) RETURN r.datetime as time ORDER BY time DESC LIMIT 1 ;"|grep -v time)
